classdef L4VehicleSimulator < handle
    %Simple Vehicle Simulator @ Luo Chun
    ...
    %
    %   OBJ = L4VehicleSimulator creates a simple simulator for a
    ...
    %
    %   OBJ = L4VehicleSimulator(MAPNAME) loads a predefined map
    ...
    %
    %   L4VehicleSimulator Published Topics:
    ...
    %
    %   L4VehicleSimulator Subscribed Topics:
    %       /mobile_base/commands/velocity - Velocity commands to the robot
    %
    %   L4VehicleSimulator Services:
    ...
    properties (Access = {?L4VehicleSimulator, ?matlab.unittest.TestCase})
    ...
    methods
        function obj = L4VehicleSimulator(varargin)
            %L4VehicleSimulator Constructor for simulator object
            %   Please check the class definition for details.
            
            obj.MyFigureTag = L4VehicleSimulator.FigureTag;
            ...
            switch nargin
                ...
                case 1
                    % L4VehicleSimulator(MAPNAME) or
                    % L4VehicleSimulator(BOG) syntax
                    map = varargin{1};
                    if ischar(map)
                        % L4VehicleSimulator(MAPNAME) syntax
                        mapResolution = 2;
                        mapName = validatestring(map, fieldnames(exampleMaps), 'L4VehicleSimulator', 'mapName');
                        ...
                        validateattributes(map, {'robotics.BinaryOccupancyGrid'}, {'nonempty','scalar'}, ...
                            'L4VehicleSimulator', 'bog');
                        obj.InternalMap = map;
                    end
                case 2
                    % L4VehicleSimulator(MAPNAME, MAPRESOLUTION) syntax
                    mapName = varargin{1};
                    mapResolution = varargin{2};
                    mapName = validatestring(mapName, fieldnames(exampleMaps), 'L4VehicleSimulator', 'mapName');
                    validateattributes(mapResolution, {'double'}, {'nonempty','scalar'}, 'L4VehicleSimulator', 'mapResolution');
                    obj.InternalMap = robotics.BinaryOccupancyGrid(exampleMaps.(mapName), mapResolution);
                otherwise
                    error('RobotSimulator:InvalidInput',...
                        'Invalid number of arguments to L4VehicleSimulator.');
            end
