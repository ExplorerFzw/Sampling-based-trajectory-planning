...
void TOS_PP::Init()
{
	...
	for (uint8_t t_i = 0; t_i < FUS_MAX_OBJ_NUM; t_i++)
	{
		...
		m_tos_obj_within_lane_st.m_tarObj_arr_st[t_i].trackingTimeWithinLane_f = 0;
	...
	for (uint8_t t_i = 0; t_i < FUS_MAX_OBJ_NUM; t_i++)//just for front object TODO
	{
		...
		if (t_lane_id <= RIGHT_RIGHT_LANE)// valid object
		{
				...
				(m_tos_obj_within_lane_st.m_tarObj_arr_st[t_i].ID_un8 == fus_output_st.objects[t_i].obj_property.id))// valid object last cycle
			{
				...
				if (m_tos_obj_within_lane_st.m_tarObj_arr_st[t_i].trackingTimeWithinLane_f < MAX_F)
				{
					m_tos_obj_within_lane_st.m_tarObj_arr_st[t_i].trackingTimeWithinLane_f += GetCycleTime_s();
				}
				else
				{
					m_tos_obj_within_lane_st.m_tarObj_arr_st[t_i].trackingTimeWithinLane_f = MAX_F;
				}
...
float TOS_PP::TTC_Calac(uint8_t t_index,FUSION_OBJ_OUTPUT_ST& fus_output_st, VEHSTATE_ST& t_vehState_st)
{
		...
		)
	{
		...
		else if (t_dRel > t_dMin)// in front
		{
			if (t_vRel < -0.3)
			{
				t_ttc = -(t_dRel - t_dFrontBumper - 0.5 * t_dobjLength) / t_vRel;
			}
			else
			{
				t_ttc = MAX_F;
		...
		else //in rear
		{
			if (t_vRel > 0.3)
			{
				t_ttc = -(t_dRel + t_dRearBumper + 0.5 * t_dobjLength) / t_vRel;
			}
			else
			{
				t_ttc = MAX_F;
		...
		if ((true == m_tos_obj_within_lane_st.m_valid[t_index]) && (t_index != MAX_UN8))
		{
			...
			case LEFT_LEFT_LANE:
			{
				if (t_count_LLLane <= TARGET_OBJECT_NUM_WITHIN_LANE)
				{
					...
					laneModel_output_st.lanes_st[LEFT_LEFT_LANE].ObjectList_st[t_count_LLLane].trackingTimeWithinLane_f = m_tos_obj_within_lane_st.m_tarObj_arr_st[t_index].trackingTimeWithinLane_f;
			...
			case LEFT_LANE:
			{
				if (t_count_LLane <= TARGET_OBJECT_NUM_WITHIN_LANE)
				{
					...
					laneModel_output_st.lanes_st[LEFT_LANE].ObjectList_st[t_count_LLane].trackingTimeWithinLane_f = m_tos_obj_within_lane_st.m_tarObj_arr_st[t_index].trackingTimeWithinLane_f;
			...
			case MIDDLE_LANE:
			{
				if (t_count_EgoLane <= TARGET_OBJECT_NUM_WITHIN_LANE)
				{
					...
					laneModel_output_st.lanes_st[MIDDLE_LANE].ObjectList_st[t_count_EgoLane].trackingTimeWithinLane_f = m_tos_obj_within_lane_st.m_tarObj_arr_st[t_index].trackingTimeWithinLane_f;
			...
			case RIGHT_LANE:
			{
				if (t_count_RLane <= TARGET_OBJECT_NUM_WITHIN_LANE)
				{
					...
					laneModel_output_st.lanes_st[RIGHT_LANE].ObjectList_st[t_count_RLane].trackingTimeWithinLane_f = m_tos_obj_within_lane_st.m_tarObj_arr_st[t_index].trackingTimeWithinLane_f;
			...
			case RIGHT_RIGHT_LANE:
			{
				if (t_count_RRLane <= TARGET_OBJECT_NUM_WITHIN_LANE)
				{
					...
					laneModel_output_st.lanes_st[RIGHT_RIGHT_LANE].ObjectList_st[t_count_RRLane].trackingTimeWithinLane_f = m_tos_obj_within_lane_st.m_tarObj_arr_st[t_index].trackingTimeWithinLane_f;
