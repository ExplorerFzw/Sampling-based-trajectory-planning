...
def generate_path(q0, q1, max_curvature):
    ...
    dy = q1[1] - q0[1]
    dth = q1[2] - q0[2] # yaw
    c = math.cos(q0[2]) # cos(yaw_start)
    s = math.sin(q0[2]) # sin(yaw_start)
    x = (c * dx + s * dy) * max_curvature  # distance between start and end point in x_direction
    y = (-s * dx + c * dy) * max_curvature # distance between start and end point in y_direction
